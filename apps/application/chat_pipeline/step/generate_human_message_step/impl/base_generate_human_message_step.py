...
class BaseGenerateHumanMessageStep(IGenerateHumanMessageStep):
    ...
    def execute(self, problem_text: str,
        ...
        system, human = self.to_message(prompt, exec_problem_text, max_paragraph_char_number, paragraph_list,
                                        no_references_setting)
        return [system, *flat_map(history_message), human]

    @staticmethod
    def to_message( prompt: str,
                    problem: str,
                    max_paragraph_char_number: int,
                    paragraph_list: List[ParagraphPipelineModel],
                    no_references_setting: Dict):
        if paragraph_list is None or len(paragraph_list) == 0:
            system = SystemMessage(content=prompt.format(data=""))
            if no_references_setting.get('status') == 'ai_questioning':
                human = HumanMessage(
                    content=no_references_setting.get('value').format(**{'question': problem}))
            else:
                human = HumanMessage(content=problem)
            return system, human
        temp_data = ""
        data_list = []
        for p in paragraph_list:
            ...
            if len(temp_data) > max_paragraph_char_number:
                # 表格不截断
                if '<table>' in content or len(content.split('|'))>2:
                ...
                data_list.append(f"<data>{content}</data>")
        data = "\n".join(data_list)
        system = SystemMessage(content=prompt.format(data=data))
        human = HumanMessage(content=problem)
        return system, human
    ...
    def to_human_message(prompt: str,
        ...
        for p in paragraph_list:
            ...
            if len(temp_data) > max_paragraph_char_number:
                # 表格不截断
                if '<table>' in content or len(content.split('|'))>=6:
                    data_list.append(f"<data>{content}</data>")
                else:
                    row_data = content[0:max_paragraph_char_number - len(temp_data)]
                    data_list.append(f"<data>{row_data}</data>")
