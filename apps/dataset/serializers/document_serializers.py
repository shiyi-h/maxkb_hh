...
from common.handle.impl.excel_split_handle import ExcelSplitHandle
...
from dataset.models.data_set import DataSet, Document, Paragraph, Problem, Type, Status, ProblemParagraphMapping, Image, LongText
...
class DocumentSerializers(ApiMixin, serializers.Serializer):
    ...
    class SplitPattern(ApiMixin, serializers.Serializer):
        @staticmethod
        def list():
            return [{'key': 'PDF：OCR', 'value': 'ocr'},
                    {'key': 'EXCEL：markdown', 'value': 'markdown'},
                    {'key': "通用：#", 'value': '(?<=^)# .*|(?<=\\n)# .*'},
                    {'key': '通用：##', 'value': '(?<=\\n)(?<!#)## (?!#).*|(?<=^)(?<!#)## (?!#).*'},
                    {'key': '通用：###', 'value': "(?<=\\n)(?<!#)### (?!#).*|(?<=^)(?<!#)### (?!#).*"},
                    {'key': '通用：####', 'value': "(?<=\\n)(?<!#)#### (?!#).*|(?<=^)(?<!#)#### (?!#).*"},
                    ...
                    {'key': '通用：######', 'value': "(?<=\\n)(?<!#)###### (?!#).*|(?<=^)(?<!#)###### (?!#).*"},
                    {'key': '通用：-', 'value': '(?<! )- .*'},
                    {'key': '通用：空格', 'value': '(?<!\\s)\\s(?!\\s)'},
                    {'key': '通用：分号', 'value': '(?<!；)；(?!；)'}, {'key': '逗号', 'value': '(?<!，)，(?!，)'},
                    {'key': '通用：句号', 'value': '(?<!。)。(?!。)'}, {'key': '回车', 'value': '(?<!\\n)\\n(?!\\n)'},
                    {'key': '通用：空行', 'value': '(?<!\\n)\\n\\n(?!\\n)'},
                    ]
...
split_handles = [HTMLSplitHandle(), DocSplitHandle(), PdfSplitHandle(), default_split_handle, ExcelSplitHandle(), PptSplitHandle()]


def save_image(image_list):
    QuerySet(Image).bulk_create(image_list)

def save_longtext(longtexts):
    QuerySet(LongText).bulk_create(longtexts)

def file_to_paragraph(file, pattern_list: List, with_filter: bool, limit: int):
    get_buffer = FileBufferHandle().get_buffer
    for split_handle in split_handles:
        if split_handle.support(file, get_buffer):
            return split_handle.handle(file, pattern_list, with_filter, limit, get_buffer, save_image, save_longtext)
    return default_split_handle.handle(file, pattern_list, with_filter, limit, get_buffer, save_image, save_longtext)
